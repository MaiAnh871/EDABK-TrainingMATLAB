%% 16QAM
%% CREATE
N4 = 100;                                   %length of b(n) signal
fs = 10000;                                 %sample frequency (Hz)
Rm = 1000;                                  %bit rate (bps)
L = fs/Rm;                                  %L = 10
bitIN = randi ([0, 1], 1, N4);              %generate b(n) signal

%% CREATE CONSTELLA DIAGRAM USING GRAY CODING
[xI, xQ] = QAM16encoder(bitIN);
...
t = (0: 1/fs: N4*L/(4*fs) - 1/fs);                   %time dimension
...
%% LPF
f = [-fs/2: 2*Rm/N4: fs/2 - 2*Rm/N4];       %reset frequency dimensions
...
Ydec = [];
for j = 10 + L/2: L: N4*fs/(4*Rm)           %j starts from 10 because of the level of the filter
